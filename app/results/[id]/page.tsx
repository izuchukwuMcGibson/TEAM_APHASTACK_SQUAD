...
import { Lock, CheckCircle, AlertTriangle, XCircle, Shield, AlertOctagon } from "lucide-react";
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    // Polling if payment was completed but report is still locked
    let interval: NodeJS.Timeout;
    if (paymentComplete && report && !report.unlocked) {
      interval = setInterval(() => {
        fetchReport();
      }, 2000);
    }
    return () => clearInterval(interval);
  }, [id, paymentComplete, report?.unlocked]);
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center">Loading...</div>;
  }

  if (!report) {
    return <div className="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center">Report not found</div>;
  }
    ...
    return (
      <main className="min-h-screen bg-slate-950 text-slate-50 flex flex-col items-center py-20 px-6">
        <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl text-center">
          <div className={`inline-flex px-4 py-2 rounded-full border mb-6 font-bold ${getRiskColor(report.preliminary.risk_level)}`}>
            RISK LEVEL: {report.preliminary.risk_level}
          </div>
          
          <h1 className="text-3xl font-bold mb-4">Verification Complete</h1>
          <p className="text-xl text-slate-300 mb-6">
            Our forensic engine detected <span className="text-[#00ff88] font-bold">{report.preliminary.flag_count}</span> suspicious signal(s) in this document
          </p>
          ...
          <div className="bg-slate-950 border border-slate-800 p-8 rounded-2xl relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-slate-900/90 z-10 flex flex-col items-center justify-end pb-8">
              ...
              <p className="text-lg font-bold text-slate-300 mb-6">Full forensic report locked</p>
              
              <form onSubmit={handleUnlock} className="flex flex-col w-full max-w-sm gap-3 px-4 z-20">
                <input
                  type="email"
                  required
                  value={unlockEmail}
                ...
                >
                  {paying ? "Processing..." : "Unlock Full Report — ₦500"}
                </button>
              </form>
            </div>
      ...
      <div className="w-full max-w-4xl space-y-8">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400">Document Type: {full.document_type_detected}</p>
          </div>
          <div className="flex gap-4 items-center">
            ...
            <div className={`px-6 py-3 rounded-2xl border font-bold text-lg ${getRiskColor(full.overall_risk)}`}>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8">
          ...
          <p className="text-slate-300 leading-relaxed text-lg">{full.summary}</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            {full.flags.map((flag: any, i: number) => (
              <div key={i} className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <div className="flex justify-between items-start mb-4">
                  <span className="font-bold text-slate-300">{flag.category}</span>
                  <span className={`text-xs font-bold px-2 py-1 rounded border ${getRiskColor(flag.severity)}`}>
                    {flag.severity}
                  </span>
                </div>
                <p className="text-slate-200 mb-2 font-medium">{flag.finding}</p>
                <p className="text-slate-400 text-sm">Implies: {flag.implication}</p>
              </div>
            ))}
            {full.flags.length === 0 && (
          ...
          <div className="space-y-6">
              ...
              <ul className="space-y-3">
                {full.authentic_signals.map((signal: string, i: number) => (
                  <li key={i} className="flex gap-3 text-slate-300">
                    <CheckCircle size={20} className="text-[#00ff88] shrink-0" />
                    <span>{signal}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-6">
              <p className="text-yellow-200 font-medium">{full.recommendation}</p>
            </div>
          </div>
        </div>
